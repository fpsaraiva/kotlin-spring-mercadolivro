...
class BookController(
    ...
    @GetMapping
    fun findAll(@PageableDefault(page = 0, size = 5) pageable: Pageable): PageResponse<BookResponse> {
        return bookService.findAll(pageable).map{ it.toResponse() }.toPageResponse()
    }

    @GetMapping("/active")
    fun findActives(@PageableDefault(page = 0, size = 5) pageable: Pageable): Page<BookResponse> {
    ...
    @PutMapping("/{id}")
    //status original deveria ser no content
